...
def GLM4(input):
    global driver
    try:
        out = send_and_update(driver, input)
    except Exception as e:
        # 打印异常信息，实际使用中可以根据需要决定是否打印
        print(f"发生错误：{e}")
        out = "报错"
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Group():
        ...
        with gr.Row():
            text0 = gr.Textbox(label=i18n("GLM-4对话输入"), value="")

            with gr.Row():
                button2 = gr.Button(i18n("发送"), variant="primary")
                button3 = gr.Button(i18n("新建对话"), variant="primary")
            with gr.Row():
                button1 = gr.Button(i18n("开启GLM-4对话"), variant="primary")
                button4 = gr.Button(i18n("关闭GLM-4"), variant="primary")

            text1 = gr.Textbox(label=i18n("GLM-4回复"), value="")
            button1.click(star, outputs=[text1], concurrency_limit=511)
            button2.click(GLM4, inputs=[text0], outputs=[text1], concurrency_limit=511) 
            button3.click(new, outputs=[text1], concurrency_limit=511)
            button4.click(close, outputs=[text1], concurrency_limit=511)
