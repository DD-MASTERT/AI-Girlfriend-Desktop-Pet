...
def glm4(preset_dialogue_file_path, user_input):
    ...
    if LLM == "kimi":
        dashscope_feedback = kimisend_and_update(driver, mes)
    else:
        dashscope_feedback = send_and_update(driver, mes)
...
def api_multi_round_conversation(input: TextInput):
    ...
    if input.text == "新建对话":
        ...
        if LLM == "kimi":
            result = kimiclick_create_text_element(driver)
        else:
            result = click_create_text_element(driver)
        # # 读取音频文件
